...
class NotificationHelper(private val context: Context) {
    ...
    private fun createChannels() {
        // قناة حالة الاختبار (منخفضة الأهمية للـ Foreground)
        ...
        // قناة نتائج الاختبار (أعلى أهمية للنتائج)
    ...
    fun showResultNotification(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            if (ContextCompat.checkSelfPermission(context, Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) {
                // الصلاحية غير ممنوحة، لا نعرض الإشعار
                return
            }
        }
    ...
    fun showTestProgressNotification(title: String, progress: Int, max: Int, isPaused: Boolean) {
    val openIntent = PendingIntent.getActivity(
        context, 0,
    ...
    val pauseResumeAction = if (isPaused) {
        Intent(context, NotificationActionReceiver::class.java).apply { action = NotificationActionReceiver.ACTION_RESUME }
    } else {
        Intent(context, NotificationActionReceiver::class.java).apply { action = NotificationActionReceiver.ACTION_PAUSE }
    }
    val pauseResumePendingIntent = PendingIntent.getBroadcast(
        context, 1, pauseResumeAction,
        PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
    )

    val cancelIntent = Intent(context, NotificationActionReceiver::class.java).apply { action = NotificationActionReceiver.ACTION_CANCEL }
    val cancelPendingIntent = PendingIntent.getBroadcast(
        context, 2, cancelIntent,
        PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
    )

    val notification = NotificationCompat.Builder(context, Constants.CHANNEL_TEST)
        .setContentTitle(title)
        .setContentText("$progress / $max")
        .setSmallIcon(R.drawable.ic_test)
        ...
        .setContentIntent(openIntent)
        .setProgress(max, progress, false)
        .addAction(R.drawable.ic_pause, if (isPaused) context.getString(R.string.notification_resume) else context.getString(R.string.notification_pause), pauseResumePendingIntent)
        .addAction(R.drawable.ic_stop, context.getString(R.string.notification_cancel), cancelPendingIntent)
        .setPriority(NotificationCompat.PRIORITY_LOW)
        .setOngoing(true)
        .build()
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
        if (ContextCompat.checkSelfPermission(context, Manifest.permission.POST_NOTIFICATIONS) == PackageManager.PERMISSION_GRANTED) {
            notificationManager.notify(Constants.NOTIFICATION_TEST_ID, notification)
        }
    } else {
        notificationManager.notify(Constants.NOTIFICATION_TEST_ID, notification)
    }
